...
class ControllerInformationTestimonial extends Controller {
	public function index() {
		...
		$description = $this->config->get('s_testimonial_description');
		...
		if ($this->config->get('s_testimonial_editor')) {
		...
		if ($this->config->get('s_testimonial_image')) {
		...
		if ($this->config->get('s_testimonial_rating')) {
			$data['sorts'][] = array(
		...
		$limits = array_unique(explode(',', $this->config->get('s_testimonial_limit')));
		...
		// Form
		if ($this->config->get('s_testimonial_guest') || $this->customer->isLogged()) {
			$data['guest'] = true;
		} else {
			$data['guest'] = false;
		}
		
		$data['image_limit'] = $this->config->get('s_testimonial_image_limit');
		
		// Captcha
		if ($this->config->get($this->config->get('config_captcha') . '_status') && $this->config->get('s_testimonial_captcha')) {
		...
		$data['date_added'] = $this->config->get('s_testimonial_date_added');
		$data['moderation'] = $this->config->get('s_testimonial_moderation');
		$data['form'] = $this->config->get('s_testimonial_form');
		$data['form_position'] = $this->config->get('s_testimonial_form_position');
		$data['field_title'] = $this->config->get('s_testimonial_title');
		$data['field_city'] = $this->config->get('s_testimonial_city');
		$data['field_email'] = $this->config->get('s_testimonial_email');
		$data['field_name'] = $this->config->get('s_testimonial_name');
		$data['field_text'] = $this->config->get('s_testimonial_text');
		$data['field_good'] = $this->config->get('s_testimonial_good');
		$data['field_bad'] = $this->config->get('s_testimonial_bad');
		$data['max_avatar'] = sprintf($this->language->get('text_max_avatar'), $this->config->get('s_testimonial_upload_avatar_width'), $this->config->get('s_testimonial_upload_avatar_height'));
		$data['max_image'] = sprintf($this->language->get('text_max_image'), $this->config->get('s_testimonial_upload_image_width'), $this->config->get('s_testimonial_upload_image_height'));
		$data['field_avatar'] = $this->config->get('s_testimonial_avatar');
	...
	private function getList() {
		...
		$data['editor'] = $this->config->get('s_testimonial_editor');
		$data['field_image'] = $this->config->get('s_testimonial_image');
		...
		foreach ($results as $result) {
			...
			if ($this->config->get('s_testimonial_avatar')) {
				
				if ($result['avatar']) {
					$avatar = $this->model_tool_image->resize($result['avatar'], $this->config->get('s_testimonial_avatar_width'), $this->config->get('s_testimonial_avatar_height'));
				} else {
					$avatar = $this->model_tool_image->resize('catalog/s_testimonial/avatar/no_avatar.png', $this->config->get('s_testimonial_avatar_width'), $this->config->get('s_testimonial_avatar_height'));
				}
			}
			...
			if ($this->config->get('s_testimonial_image')) {
				...
				if ($result['image']) {
					...
					foreach (explode('|', $result['image']) as $img) {
						$image[] = array(
							'thumbnail' => $this->model_tool_image->resize($img, $this->config->get('s_testimonial_thumbnail_width'), $this->config->get('s_testimonial_thumbnail_height')),
							'thumb'     => $this->model_tool_image->resize($img, $this->config->get('s_testimonial_thumb_width'), $this->config->get('s_testimonial_thumb_height'))
						);
					}
				}
			}
			
			$text_limit = $this->config->get('s_testimonial_text_limit');
			
			$text = '';
			$readmore = false;
			
			if ($this->config->get('s_testimonial_text')) {
				...
				if ($this->config->get('s_testimonial_cut') && mb_strlen(strip_tags($text), 'UTF-8') > $text_limit) {
			...
			if ($this->config->get('s_testimonial_good')) {
				...
				if ($this->config->get('s_testimonial_cut') && mb_strlen(strip_tags($good), 'UTF-8') > $text_limit) {
			...
			if ($this->config->get('s_testimonial_bad')) {
				...
				if ($this->config->get('s_testimonial_cut') && mb_strlen(strip_tags($bad), 'UTF-8') > $text_limit) {
			...
			$data['testimonials'][] = array(
				...
				'title'    	 	 => $this->config->get('s_testimonial_title') ? $result['title'] : '',
				'city'			 => $this->config->get('s_testimonial_city') ? $result['city'] : '',
				'name'		 	 => $this->config->get('s_testimonial_name') ? $result['name'] : '',
				'text'		 	 => $text,
				'good'		 	 => $good,
				'bad'		 	 => $bad,
				'rating'	 	 => $this->config->get('s_testimonial_rating') ? $result['rating'] : '',
				'avatar'	 	 => $avatar,
				'image'	 		 => $image,
				...
				'date_added'	 => $this->config->get('s_testimonial_date_added') ? date($this->language->get('date_format_short'), strtotime($result['date_added'])) : '',
	...
	private function getTotal() {
		...
		$data['field_rating'] = $this->config->get('s_testimonial_rating');
	...
	public function info() {
		...
		if ($testimonial_info) {
			...
			if ($this->config->get('s_testimonial_avatar') || $this->config->get('s_testimonial_image')) {
				$this->document->addStyle('catalog/view/javascript/jquery/magnific/magnific-popup.css');
				$this->document->addScript('catalog/view/javascript/jquery/magnific/jquery.magnific-popup.min.js');
			...
			if ($this->config->get('s_testimonial_avatar')) {
				if ($testimonial_info['avatar']) {
					$data['avatar'] = $this->model_tool_image->resize($testimonial_info['avatar'], $this->config->get('s_testimonial_avatar_info_width'), $this->config->get('s_testimonial_avatar_info_height'));
				} else {
					$data['avatar'] = $this->model_tool_image->resize('catalog/s_testimonial/avatar/no_avatar.png', $this->config->get('s_testimonial_avatar_info_width'), $this->config->get('s_testimonial_avatar_info_height'));
				}
			}
			...
			if ($this->config->get('s_testimonial_image')) {
				...
				if ($testimonial_info['image']) {
					...
					foreach (explode('|', $testimonial_info['image']) as $value) {
						$data['image'][] = array(
							'thumbnail' => $this->model_tool_image->resize($value, $this->config->get('s_testimonial_thumbnail_info_width'), $this->config->get('s_testimonial_thumbnail_info_height')),
							'thumb'     => $this->model_tool_image->resize($value, $this->config->get('s_testimonial_thumb_width'), $this->config->get('s_testimonial_thumb_height'))
						);
					}
				}
			}
				
			$data['title'] = $this->config->get('s_testimonial_title') ? $testimonial_info['title'] : '';
			$data['city'] = $this->config->get('s_testimonial_city') ? $testimonial_info['city'] : '';
			$data['name'] = $this->config->get('s_testimonial_name') ? $testimonial_info['name'] : '';
			$data['text'] = $this->config->get('s_testimonial_text') ? ($this->config->get('s_testimonial_editor') ? $this->model_catalog_s_testimonial->replaceBBCode($testimonial_info['text']) : $testimonial_info['text']) : '';
			$data['good'] = $this->config->get('s_testimonial_good') ? ($this->config->get('s_testimonial_editor') ? $this->model_catalog_s_testimonial->replaceBBCode($testimonial_info['good']) : $testimonial_info['good']) : '';
			$data['bad'] = $this->config->get('s_testimonial_bad') ? ($this->config->get('s_testimonial_editor') ? $this->model_catalog_s_testimonial->replaceBBCode($testimonial_info['bad']) : $testimonial_info['bad']) : '';
			...
			$data['rating'] = $this->config->get('s_testimonial_rating') ? $testimonial_info['rating'] : '';
			$data['date_added'] = $this->config->get('s_testimonial_date_added') ? date($this->language->get('date_format_short'), strtotime($testimonial_info['date_added'])) : '';
	...
	private function checkFile($folder, $file) {
		...
		if ($folder == 'avatar') {
			$width = $this->config->get('s_testimonial_upload_avatar_width');
			$height = $this->config->get('s_testimonial_upload_avatar_height');
		} else {
			$width = $this->config->get('s_testimonial_upload_image_width');
			$height = $this->config->get('s_testimonial_upload_image_height');
		}
	...
	public function write() {
		if ($this->config->get('s_testimonial_status') && ($this->config->get('s_testimonial_guest') || $this->customer->isLogged())) { 
			$this->load->language('information/testimonial');
			...
			if ($this->request->server['REQUEST_METHOD'] == 'POST') {
				...
				if ($this->config->get('s_testimonial_avatar') == 2 && (empty($this->request->files['avatar']['name']) || !is_file($this->request->files['avatar']['tmp_name']))) {	
					$json['error'] = $this->language->get('error_avatar');
				} elseif ($this->config->get('s_testimonial_avatar') && !empty($this->request->files['avatar']['name']) && is_file($this->request->files['avatar']['tmp_name'])) {
				...
				if ($this->config->get('s_testimonial_image') && isset($this->request->files['image']) && $this->request->files['image']) {
				...
				if (!isset($json['error'])) {
					...
					if ($this->config->get('s_testimonial_moderation')) {
						$data['status'] = 0;
