...
class ModelCatalogSTestimonial extends Model {
	...
	public function getTestimonials($data = array()) {
		if (!$this->config->get('s_testimonial_language')) {
			$language = "st.language_id = '" . (int)$this->config->get('config_language_id') . "' AND ";
		} else {
			$language =  "";
		}
		
		if ($this->config->get('s_testimonial_all_store')) {
	...
	public function addTestimonial($data) {
		...
		if ($this->config->get('s_testimonial_title') && isset($data['title']) && $data['title']) { 
			$sql .= "title = '" . $this->db->escape($data['title']) . "', "; 
		}
		
		if ($this->config->get('s_testimonial_city') && isset($data['city']) && $data['city']) { 
			$sql .= "city = '" . $this->db->escape($data['city']) . "', "; 
		}
		
		if ($this->config->get('s_testimonial_email') && isset($data['email']) && $data['email']) { 
			$sql .= "email='" . $this->db->escape($data['email']) . "', "; 
		}
		
		if ($this->config->get('s_testimonial_name') && isset($data['name']) && $data['name']) { 
			$sql .= "name='" . $this->db->escape($data['name']) . "', "; 
		}
		
		if ($this->config->get('s_testimonial_text') && isset($data['text']) && $data['text']) { 
			$sql .= "text='" . $this->db->escape($data['text']) . "', "; 
		}
		
		if ($this->config->get('s_testimonial_good') && isset($data['good']) && $data['good']) { 
			$sql .= "good='" . $this->db->escape($data['good']) . "', "; 
		}
		
		if ($this->config->get('s_testimonial_bad') && isset($data['bad']) && $data['bad']) { 
			$sql .= "bad='" . $this->db->escape($data['bad']) . "', "; 
		}
		
		if ($this->config->get('s_testimonial_rating') && isset($data['rating']) && $data['rating']) { 
			$sql .= "rating='" . $this->db->escape($data['rating']) . "', "; 
		}
		
		if ($this->config->get('s_testimonial_avatar') && isset($data['avatar']) && $data['avatar']) { 
			$sql .= "avatar='" . $this->db->escape($data['avatar']) . "', "; 
		}
		
		if ($this->config->get('s_testimonial_image') && isset($data['image']) && $data['image']) { 
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'testimonial_id=" . (int)$testimonial_id . "', keyword = '" . $this->db->escape($this->config->get('s_testimonial_keyword')) . "-" . (int)$testimonial_id . "'");

		if ($this->config->get('s_testimonial_mail')) {
			...
			if ($this->config->get('s_testimonial_text') && isset($data['text']) && $data['text']) {
				...
				$html['text'] = $this->config->get('s_testimonial_editor') ? $this->replaceBBCode($data['text']) : $data['text'];
			}
	...
	public function getSmiles() {
		$theme = $this->config->get('s_testimonial_smile_theme');
