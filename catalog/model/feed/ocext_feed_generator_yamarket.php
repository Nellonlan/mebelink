<?php
class ModelFeedOcextFeedGeneratorYaMarket extends Model {
    
    /*
     * 
    public function getFilterData($key) {
        $sql = "SELECT * FROM `" . DB_PREFIX . "ocext_feed_generator_yamarket_filter_data` WHERE `key` = '".$key."' ";
    ...
    public function getCategoriesAndProducts($ym_categories,$ym_manufacturers,$filter_data_group_id,$content_language_id) {
        $p2c = '';
        $ym_attributes = array();
        $all_yml_export_ocext_ym_filter_data_attributes = $this->model_feed_ocext_feed_generator_yamarket->getFilterData('ocext_feed_generator_yamarket_ym_filter_attributes',$filter_data_group_id,$content_language_id);
        if($all_yml_export_ocext_ym_filter_data_attributes){
            $ym_attributes = $all_yml_export_ocext_ym_filter_data_attributes;
        }
        $ym_options = array();
        $all_yml_export_ocext_ym_filter_data_options = $this->model_feed_ocext_feed_generator_yamarket->getFilterData('ocext_feed_generator_yamarket_ym_filter_options',$filter_data_group_id,$content_language_id);
        ...
        if($query->rows){
            foreach ($query->rows as $product) {
                ...
                if($ym_categories){
                    foreach ($ym_categories as $category_id => $ym_category) {
                        if($category_id==$product['category_id'] && isset($ym_category['setting_id']) && $ym_category['setting_id']){
                            $set[$ym_category['setting_id']] = $ym_category['setting_id'];
                        }
                    }
                }else{
                    //Если для всех категорий отсутствует - то нулевой - упращенный фид
                    $set[0]='0';
                }
                    ...
                    if($ym_manufacturers){
                        foreach ($ym_manufacturers as $manufacturer_id => $ym_manufacturer) {
                            if($manufacturer_id==$product['manufacturer_id']){
                                $set[$ym_manufacturer['setting_id']] = $ym_manufacturer['setting_id'];
                            }
                        }
                    }else{
                        //Если для всех производителей отсутствует - то нулевой - упращенный фид
                        $set[0]='0';
                    }
