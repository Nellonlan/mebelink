...
class ControllerCatalogSTestimonial extends Controller { 
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('catalog/s_testimonial', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	private function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('catalog/s_testimonial', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('catalog/s_testimonial/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('catalog/s_testimonial/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		// Show Field
		if ($this->config->get('s_testimonial_field')) {
    	...
    	foreach ($results as $result) {	
			...
			$text = $this->config->get('s_testimonial_editor') ? $this->model_catalog_s_testimonial->clearBBCode($result['text']) : $result['text'];
			
			if (mb_strlen($text,'UTF-8') > 100){
				$text = utf8_substr($text, 0, 100) . '...';
			}
			
			$good = $this->config->get('s_testimonial_editor') ? $this->model_catalog_s_testimonial->clearBBCode($result['good']) : $result['good'];
			
			if (mb_strlen($good,'UTF-8') > 100){
				$good = utf8_substr($good, 0, 100) . '...';
			}
			
			$bad = $this->config->get('s_testimonial_editor') ? $this->model_catalog_s_testimonial->clearBBCode($result['bad']) : $result['bad'];
			...
			$data['testimonials'][] = array(
				...
				'edit'     	     => $this->url->link('catalog/s_testimonial/edit', 'token=' . $this->session->data['token'] . '&testimonial_id=' . $result['testimonial_id'] . $url, true)
			);
		}	
		...
		$data['sort_title'] = $this->url->link('catalog/s_testimonial', 'token=' . $this->session->data['token'] . '&sort=title' . $url, true);			
		$data['sort_text'] = $this->url->link('catalog/s_testimonial', 'token=' . $this->session->data['token'] . '&sort=text' . $url, true);
		$data['sort_good'] = $this->url->link('catalog/s_testimonial', 'token=' . $this->session->data['token'] . '&sort=good' . $url, true);
		$data['sort_bad'] = $this->url->link('catalog/s_testimonial', 'token=' . $this->session->data['token'] . '&sort=bad' . $url, true);
		$data['sort_name'] = $this->url->link('catalog/s_testimonial', 'token=' . $this->session->data['token'] . '&sort=name' . $url, true);
		$data['sort_city'] = $this->url->link('catalog/s_testimonial', 'token=' . $this->session->data['token'] . '&sort=city' . $url, true);
		$data['sort_rating'] = $this->url->link('catalog/s_testimonial', 'token=' . $this->session->data['token'] . '&sort=rating' . $url, true);
		$data['sort_language_id'] = $this->url->link('catalog/s_testimonial', 'token=' . $this->session->data['token'] . '&sort=language_id' . $url, true);
		$data['sort_email'] = $this->url->link('catalog/s_testimonial', 'token=' . $this->session->data['token'] . '&sort=email' . $url, true);			
		$data['sort_date_added'] = $this->url->link('catalog/s_testimonial', 'token=' . $this->session->data['token'] . '&sort=date_added' . $url, true);		
		$data['sort_status'] = $this->url->link('catalog/s_testimonial', 'token=' . $this->session->data['token'] . '&sort=status' . $url, true);	
		...
		$pagination->url = $this->url->link('catalog/s_testimonial', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
	...
	private function getForm() {
		...
		if (!isset($this->request->get['testimonial_id'])) {
			$data['action'] = $this->url->link('catalog/s_testimonial/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('catalog/s_testimonial/edit', 'token=' . $this->session->data['token'] . '&testimonial_id=' . $this->request->get['testimonial_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('catalog/s_testimonial', 'token=' . $this->session->data['token'] . $url, true);
		...
		if ($this->config->get('s_testimonial_field')) {
			...
			$data['field_title'] = $this->config->get('s_testimonial_title') ? $this->config->get('s_testimonial_title') : 1;
			$data['field_city'] = $this->config->get('s_testimonial_city') ? $this->config->get('s_testimonial_city') : 1;
			$data['field_name'] = $this->config->get('s_testimonial_name') ? $this->config->get('s_testimonial_name') : 1;
			$data['field_email'] = $this->config->get('s_testimonial_email') ? $this->config->get('s_testimonial_email') : 1;
			$data['field_text'] = $this->config->get('s_testimonial_text') ? $this->config->get('s_testimonial_text') : 1;
			$data['field_good'] = $this->config->get('s_testimonial_good') ? $this->config->get('s_testimonial_good') : 1;
			$data['field_bad'] = $this->config->get('s_testimonial_bad') ? $this->config->get('s_testimonial_bad') : 1;
			$data['field_rating'] = $this->config->get('s_testimonial_rating') ? $this->config->get('s_testimonial_rating') : 1;
			$data['field_avatar'] = $this->config->get('s_testimonial_avatar') ? $this->config->get('s_testimonial_avatar') : 1;
			$data['field_image'] = $this->config->get('s_testimonial_image') ? $this->config->get('s_testimonial_image') : 1;
		}
	...
	private function validateForm() {
		...
		if (utf8_strlen($this->request->post['keyword']) > 0) {
			...
			if ($url_alias_info && isset($this->request->get['testimonial_id']) && $url_alias_info['query'] != 'testimonial_id=' . $this->request->get['testimonial_id']) {
				$this->error['keyword'] = sprintf($this->language->get('error_keyword'));
			}

			if ($url_alias_info && !isset($this->request->get['testimonial_id'])) {
				$this->error['keyword'] = sprintf($this->language->get('error_keyword'));
			}
		}
