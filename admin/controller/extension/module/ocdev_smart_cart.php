...
class ControllerExtensionModuleOCdevSmartCart extends Controller {
  ...
  public function index() {
    ...
    $models = array('setting/store', 'setting/setting', 'extension/extension', 'catalog/category', 'catalog/manufacturer', 'customer/customer_group', 'localisation/language', 'catalog/product', 'tool/image', 'extension/module/'.self::$_module_name);
    ...
    $data['breadcrumbs'] = array(
      ...
      2 => array(
        ...
        'href'      => $this->url->link('extension/module/'.self::$_module_name, 'token='.$this->session->data['token'], true),
        'separator' => ' :: '
      )
    );

    $data['action']           = $this->url->link('extension/module/'.self::$_module_name, 'token='.$this->session->data['token'], true);
    $data['action_plus']      = $this->url->link('extension/module/'.self::$_module_name.'/edit_and_stay', 'token='.$this->session->data['token'], true);
    $data['export_settings_button'] = $this->url->link('extension/module/'.self::$_module_name.'/export_settings', 'token='.$this->session->data['token'], true);
    $data['import_settings_button'] = $this->url->link('extension/module/'.self::$_module_name.'/import_settings', 'token='.$this->session->data['token'], true);
  ...
  public function history() {
    ...
    $pagination->limit = 10;
    $pagination->url = $this->url->link('extension/module/'.self::$_module_name.'/history', 'token='.$this->session->data['token'].'&page={page}', true);
  ...
  public function edit_and_stay() {
    ...
    if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validate()) {
      ...
      if (isset($content)) {
        $this->session->data['success'] = $this->language->get('text_success');
        $this->model_setting_setting->editSetting(self::$_module_name, unserialize($content));
        $this->response->redirect($this->url->link('extension/module/'.self::$_module_name, 'token='.$this->session->data['token'], true));
      } else {
        $this->session->data['success'] = $this->language->get('text_success');
        $this->model_setting_setting->editSetting(self::$_module_name, $this->request->post);
        $this->response->redirect($this->url->link('extension/module/'.self::$_module_name, 'token='.$this->session->data['token'], true));
  ...
  public function install() {
    ...
    // connect model data
    $models = array('setting/setting', 'extension/extension', 'extension/module/'.self::$_module_name);
    foreach ($models as $model) {
      $this->load->model($model);
    }
