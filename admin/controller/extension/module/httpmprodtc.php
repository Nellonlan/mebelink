	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->model_extension_module->addModule('httpmprodtc', $this->request->post);
			} else {
...
$data['tab_inactive'] = $this->language->get('tab_inactive');
...
$data['entry_tname'] = $this->language->get('entry_tname');
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('module/httpmprodtc', 'token=' . $this->session->data['token'], 'SSL')
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('module/httpmprodtc', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/httpmprodtc', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('extension/module/httpmprodtc', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
		...
		if (isset($this->request->get['module_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			...
			$module_info = array();
			$module_info['tabs'] = array(
				array('id' => 1,'product' => '','tab'=>array(1), 'tname'=> '', 'category'=>''),array('id' => 1,'product' => '','tab'=>array(1), 'tname'=> '', 'category'=>'')
			);
			$module_info['items'] = '';
			$module_info['name'] = '';
			$module_info['class'] = '';
			$module_info['limit'] = '';
			$module_info['prodview'] = '';
			$module_info['width'] = '';
			$module_info['height'] = '';
			$module_info['status'] = '0';
			$module_info['displaytitle'] = '1';
		}
		...
		$data['tabs']=array();
		$i=0;
		foreach ($module_info['tabs'] as $tab) {
			$i++;
			$tab['id'] = $i;
			$products = array();
			if ($tab['product']) {
				foreach ($tab['product'] as $product_id) {
		...
		$this->response->setOutput($this->load->view('extension/module/httpmprodtc.tpl', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/httpmprodtc')) {
