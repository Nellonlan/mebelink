<?php
class ControllerFeedOcextFeedGeneratorYaMarket extends Controller {
    /*
	private $this_version = '2.3.0.0';
...
*/
		...
		$this->load->language('feed/ocext_feed_generator_yamarket');
		...
		$this->document->setTitle($this->language->get('heading_title'));
                ...
                $this->load->model('feed/ocext_feed_generator_yamarket');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
                        ...
                        if(isset($this->request->get['template_setting'])){
                            $this->model_feed_ocext_feed_generator_yamarket->setSettings($this->request->post);
                            $this->session->data['success'] = $this->language->get('text_success');
                            
                            $this->response->redirect($this->url->link('feed/ocext_feed_generator_yamarket', $url.'&token=' . $this->session->data['token'], 'SSL'));
                            ...
                            if(!$filter_data_group_id){
                                $data['filter_datas'] = $this->model_feed_ocext_feed_generator_yamarket->getFilterDatas();
                            ...
                            $this->model_feed_ocext_feed_generator_yamarket->setFilterData('ocext_feed_generator_yamarket_ym_filter_attributes',$ocext_feed_generator_yamarket_ym_filter_attributes,$filter_data_name,$filter_data_group_id);

                            $ocext_feed_generator_yamarket_ym_filter_options = $this->request->post['ocext_feed_generator_yamarket_ym_filter_options'];
                            $this->model_feed_ocext_feed_generator_yamarket->setFilterData('ocext_feed_generator_yamarket_ym_filter_options',$ocext_feed_generator_yamarket_ym_filter_options,$filter_data_name,$filter_data_group_id);
                            ...
                            $this->model_feed_ocext_feed_generator_yamarket->setFilterData('ocext_feed_generator_yamarket_ym_filter_manufacturers',$ocext_feed_generator_yamarket_ym_filter_manufacturers,$filter_data_name,$filter_data_group_id);

                            $ocext_feed_generator_yamarket_ym_filter_category = $this->request->post['ocext_feed_generator_yamarket_ym_filter_category'];
                            $this->model_feed_ocext_feed_generator_yamarket->setFilterData('ocext_feed_generator_yamarket_ym_filter_category',$ocext_feed_generator_yamarket_ym_filter_category,$filter_data_name,$filter_data_group_id);
                            ...
                            $this->model_feed_ocext_feed_generator_yamarket->updateYmCategories($this->request->post);
                            $this->session->data['success'] = $this->language->get('text_success');
                ...
                $this->load->model('feed/ocext_feed_generator_yamarket');
                $data['template_setting'] = $this->model_feed_ocext_feed_generator_yamarket->getSettings($data['setting_type'],FALSE,$setting_product_id);
		...
		$languages = $this->model_localisation_language->getLanguages();
                ...
                $data['filter_datas_general_setting'] = $this->model_feed_ocext_feed_generator_yamarket->getFilterDatas();
                ...
                if($setting_product_id){
                    ...
                    return $this->load->view('feed/feed_generator/ocext_feed_generator_yamarket_product_setting.tpl', $data);
		...
		foreach ($results as $result) {
			...
			'limit'                => $config_limit_admin_pagination
                ...
                $data['ym_categories'] = $this->model_feed_ocext_feed_generator_yamarket->getYmCategoriesFromDb($filter);
		$ym_categories_total = $this->model_feed_ocext_feed_generator_yamarket->getYmCategoriesFromDbTotal($filter);
                ...
                $pagination->url = $this->url->link('feed/ocext_feed_generator_yamarket', 'token=' . $this->session->data['token'] . $url . '&ym_categories_page={page}', 'SSL');
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('feed/ocext_feed_generator_yamarket', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
                ...
                $url = '';
                $data['action_general_setting'] = $this->url->link('feed/ocext_feed_generator_yamarket', $url.'&general_setting=1&token=' . $this->session->data['token'], 'SSL');
                $data['action_template_setting'] = $this->url->link('feed/ocext_feed_generator_yamarket', $url.'&template_setting=1&token=' . $this->session->data['token'], 'SSL');
                $data['action_ym_filter_data'] = $this->url->link('feed/ocext_feed_generator_yamarket', $url.'&ym_filter_data=1&token=' . $this->session->data['token'], 'SSL');
                $data['action_ym_categories_filter'] = $this->url->link('feed/ocext_feed_generator_yamarket', $url.'&ym_categories_filter=1&token=' . $this->session->data['token'], 'SSL');
                $data['action_ym_categories'] = $this->url->link('feed/ocext_feed_generator_yamarket', $url.'&ym_categories=1&token=' . $this->session->data['token'], 'SSL');
		...
		if (version_compare(VERSION, '2.2') < 0) {
			$this->response->setOutput($this->load->view('feed/ocext_feed_generator_yamarket.tpl', $data));
		} else {
			$this->response->setOutput($this->load->view('feed/ocext_feed_generator_yamarket', $data));
		}
        ...
        public function getTemplateSetting() {
            ...
            if($sample_setting_id){
                ...
                $data['template_setting'] = $this->model_feed_ocext_feed_generator_yamarket->getSettings(FALSE,$sample_setting_id);
                
            }else{
                
                $data['template_setting'] = $this->model_feed_ocext_feed_generator_yamarket->getSettings($setting_type,$setting_id);
            ...
            $data['all_template_setting'] = $this->model_feed_ocext_feed_generator_yamarket->getSettings();
            ...
            $data['offer_description_parts'] = $this->model_feed_ocext_feed_generator_yamarket->getOfferNameParts(TRUE);
            ...
            $data['debug'] = $this->debug;
            $this->response->setOutput($this->load->view('feed/feed_generator/ocext_feed_generator_yamarket_setting_form.tpl', $data));
        ...
        public function getSettingFields() {
            ...
            $data['content_parts'] = $this->model_feed_ocext_feed_generator_yamarket->getContentParts($data['name_field'],$divide_on_option_option_id);
            $data['setting_fields'] = $this->model_feed_ocext_feed_generator_yamarket->getSettingFields($data['content_parts']);
            $data['attributes'] = $this->model_feed_ocext_feed_generator_yamarket->getAttributes();
            $data['options'] = $this->model_feed_ocext_feed_generator_yamarket->getOptions();
            ...
            if($sample_setting_id){
                ...
                $data['template_setting'] = $this->model_feed_ocext_feed_generator_yamarket->getSettings($setting_type,$data['setting_id']);
            ...
            $data['debug'] = $this->debug;
            $this->response->setOutput($this->load->view('feed/feed_generator/ocext_feed_generator_yamarket_setting_fields.tpl', $data));
        }

        private function validate() {
            
		if (!$this->user->hasPermission('modify', 'feed/ocext_feed_generator_yamarket')) {
        ...
        public function getYmFilterData() {
            ...
            if(isset($this->request->get['categories'])){
                ...
                $data['ym_categories'] = $this->model_feed_ocext_feed_generator_yamarket->getFilterData('ocext_feed_generator_yamarket_ym_filter_category',FALSE,$filter_data_group_id);
            }elseif(isset($this->request->get['manufacturers'])){
                $this->load->model('catalog/manufacturer');
                $data['manufacturers'] = $this->model_catalog_manufacturer->getManufacturers();
                $data['ym_manufacturers'] = $this->model_feed_ocext_feed_generator_yamarket->getFilterData('ocext_feed_generator_yamarket_ym_filter_manufacturers',FALSE,$filter_data_group_id);
            }elseif(isset($this->request->get['options'])){
                $data['options'] = $this->model_feed_ocext_feed_generator_yamarket->getOptions();
                $data['ym_options'] = $this->model_feed_ocext_feed_generator_yamarket->getFilterData('ocext_feed_generator_yamarket_ym_filter_options',FALSE,$filter_data_group_id);
            }elseif(isset($this->request->get['attributes'])){
                $data['attributes'] = $this->model_feed_ocext_feed_generator_yamarket->getAttributes();
                $data['ym_attributes'] = $this->model_feed_ocext_feed_generator_yamarket->getFilterData('ocext_feed_generator_yamarket_ym_filter_attributes',FALSE,$filter_data_group_id);
            }
            ...
            $data['setting_type'] = 'template_setting';
            $this->load->model('feed/ocext_feed_generator_yamarket');
            $data['templates_setting'] = $this->model_feed_ocext_feed_generator_yamarket->getSettings($data['setting_type'],FALSE,0);
            ...
            $this->response->setOutput($this->load->view('feed/feed_generator/ocext_feed_generator_yamarket_ym_filter_data.tpl', $data));
        }
        ...
        public function getCategories() {
            ...
            $ym_categories = $this->model_feed_ocext_feed_generator_yamarket->getYmCategoriesFromDb($filter_ym_categories);
            ...
            $this->response->setOutput($this->load->view('feed/feed_generator/ocext_feed_generator_yamarket_ym_categories_categories.tpl', $data));
        }
        
        public function getNotifications() {
                $module_name = 'feed/ocext_feed_generator_yamarket';
		sleep(1);
	...
	public function getNotificationsCurl() {
        ...
        public function getNewCompositeElement() {
            ...
            $data['text_template_setting_name_composite_element_seo_title'] = $this->language->get('text_template_setting_name_composite_element_meta_title');
            $data['text_template_setting_name_composite_element_meta_h1'] = 'Meta H1 товара';
            $data['text_template_setting_name_composite_element_seo_h1'] = 'SEO H1 товара';
            ...
            $data['text_disable'] = $this->language->get('text_disable');
            $data['text_enable'] = $this->language->get('text_enable');
            $data['setting_id'] = (int)$this->request->get['setting_id'];
            $data['sort_order'] = $this->request->get['sort_order'];
            $data['setting_type'] = $this->request->get['setting_type'];
            $data['template_setting'] = $this->model_feed_ocext_feed_generator_yamarket->getSettings($data['setting_type'],$data['setting_id']);
            if($data['template_setting']){
            ...
            $data['content_parts'] = $this->model_feed_ocext_feed_generator_yamarket->getContentParts();
            $data['attributes'] = $this->model_feed_ocext_feed_generator_yamarket->getAttributes();
            $data['options'] = $this->model_feed_ocext_feed_generator_yamarket->getOptions();
            $this->response->setOutput($this->load->view('feed/feed_generator/ocext_feed_generator_yamarket_setting_offer_composite.tpl', $data));
        }
        */
