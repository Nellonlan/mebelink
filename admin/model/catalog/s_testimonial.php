...
class ModelCatalogSTestimonial extends Model {
	public function addTestimonial($data) {
		...
		if ($data['keyword']) {
			...
			if ($this->config->get('config_seo_url') && ($this->config->get('config_seo_url_type') == 'seo_pro')) {
	...
	public function getTestimonial($testimonial_id) {
		$query = $this->db->query("SELECT DISTINCT *, (SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = 'testimonial_id=" . (int)$testimonial_id . "') AS keyword FROM " . DB_PREFIX . "s_testimonial st WHERE st.testimonial_id = '" . (int)$testimonial_id . "'");
	...
	public function editTestimonialUrlAlias($keyword) {
		if ($keyword) {
			$this->db->query("DELETE FROM " . DB_PREFIX . "url_alias WHERE query = 'information/testimonial'");
			
			$this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'information/testimonial', keyword = '" . $this->db->escape($keyword) . "'");
		}
	}
	...
	public function updateTestimonials() {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "url_alias WHERE query = 'product/testimonial'");
