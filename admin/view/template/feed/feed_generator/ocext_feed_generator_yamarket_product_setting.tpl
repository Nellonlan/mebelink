...
function setSettingYamarket(){
    ...
    $.ajax({
            type: 'post',
            url: 'index.php?route=feed/ocext_feed_generator_yamarket/setSettings&&token=<?php echo $token; ?>',
            dataType: 'json',
...
function  settingFieldsYamarket(value_selected,setting_id,name_field){
    $('#form-ocext_feed_generator_yamarket .setting_fields'+setting_id+name_field).hide();
    if(value_selected=='attribute_id' || value_selected=='category_id' || value_selected=='option_id' || value_selected=='composite' || value_selected=='text_field'){
        $('#form-ocext_feed_generator_yamarket #setting_fields_'+value_selected+setting_id+name_field).show();
    }
}
