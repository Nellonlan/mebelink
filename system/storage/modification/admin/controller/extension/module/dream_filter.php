...
class ControllerExtensionModuleDreamFilter extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'] = array(
			...
			array(
				...
				'href' => $this->url->link('extension/extension', 'token=' . $token . '&type=module', true)
			)
		);
		if(isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				'text' => $language['heading_title'],
				'href' => $this->url->link('extension/module/dream_filter', 'token=' . $token . '&module_id=' . $this->request->get['module_id'], true)
			);
			$location = $this->url->link('extension/module/dream_filter/edit', 'module_id=' . $this->request->get['module_id'] . '&token=' . $token, true);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $language['heading_title'],
				'href' => $this->url->link('extension/module/dream_filter', 'token=' . $token, true)
			);
			$location = $this->url->link('extension/module/dream_filter/edit', 'token=' . $token, true);
		}
        ...
        if(version_compare(phpversion(), '5.6', '<')) {
            ...
            $data['errors'][] = sprintf($language['error_modification'], $this->url->link('extension/modification', 'token=' . $token, true));
        } else {
            ...
            $data['activate'] = $this->url->link('extension/module/dream_filter/activate', 'token=' . $token, true);
        }
	...
	public function edit() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validate($this->request->post, $languages)) {
			...
			$this->response->redirect($this->url->link('extension/extension', 'token=' . $token . '&type=module', true));
		}
		...
		if($module_id && empty($filter_layouts)) {
			if(empty($this->error['warning'])) {
				$this->error['warning'] = sprintf($language['error_layout'], $this->url->link('design/layout', 'token=' . $token, true));
			}
		}
		...
		$data['breadcrumbs'] = array(
			...
			array(
				'text' => $language['heading_title'],
				'href' => $this->url->link('extension/module/dream_filter/edit', 'token=' . $token . ($module_id ? '&module_id=' . $module_id : ''), true)
			)
		);
		$data['action'] = $this->url->link('extension/module/dream_filter/edit', 'token=' . $token . ($module_id ? '&module_id=' . $module_id : ''), true);
		$data['home'] = $this->url->link('extension/module/dream_filter', 'token=' . $token . ($module_id ? '&module_id=' . $module_id : ''), true);
		$data['apply'] = $module_id ? $this->url->link('extension/module/dream_filter/apply', 'module_id=' . $module_id . '&token=' . $token, true) : false;
		...
		$data['cancel'] = $this->url->link('extension/extension', 'token=' . $token . '&type=module', true);
		$data['activate'] = $this->url->link('extension/module/dream_filter/activate', 'token=' . $token, true);
		$data['cache_btn'] = $this->url->link('extension/module/dream_filter/cache', 'token=' . $token, true);
		$data['reset_btn'] = $this->url->link('extension/module/dream_filter/reset', 'token=' . $token, true);
		$data['updates_btn'] = $this->url->link('extension/module/dream_filter/updates', 'token=' . $token, true);
		$data['token'] = $token;
		...
		$form_new = array(
			'id' => 'new',
			'action' => $this->url->link('extension/module/dream_filter/filter', 'token=' . $token . ($module_id ? '&module_id=' . $module_id : ''), true),
			'token' => $token,
		...
		if(!empty($data['filters']) && is_array($data['filters'])) {
		    ...
		    foreach ($data['filters'] as $id => $filter) {
				...
				$filter_forms[] = $this->load->view('extension/module/dream_filter_form', array(
					'id' => $id,
					'action' => $this->url->link('extension/module/dream_filter/filter', 'token=' . $token . ($module_id ? '&module_id=' . $module_id : '') . '&id=' . $id, true),
					'token' => $token,
	...
	public function activate() {
        ...
        $json = $this->model_extension_dream_filter->getLicense($store_id);
		if(!isset($this->request->get['module_id'])) {
	...
	public function updates() {
		$this->load->model('extension/dream_filter');
        $json = $this->model_extension_dream_filter->getUpdates();
		$this->response->addHeader('Content-Type: application/json');
		$this->response->setOutput(json_encode($json));
	}
	...
	public function filter() {
		...
		if(!isset($this->request->get['id']) && !isset($json['errors'])) {
			...
			$json['modal'] = $this->load->view('extension/module/dream_filter_form', array(
				'id' => $json['id'],
				'action' => $this->url->link('extension/module/dream_filter/filter', 'token=' . $token . ($module_id ? '&module_id=' . $module_id : '') . '&id=' . $json['id'], true),
				'token' => $token,
