...
'filter_date'     => $filter_date,
      ...
      $category =  $this->model_catalog_product->getProductCategories($result['product_id']);
		...
		$data['entry_name'] = $this->language->get('entry_name');
				...
				$data['entry_mrp'] 		= $this->language->get('entry_mrp');
				$data['button_remove'] = $this->language->get('button_remove');
				$data['entry_add_table'] = $this->language->get('entry_add_table');
				$data['entry_title'] = $this->language->get('entry_title');
				$data['entry_table_product'] = $this->language->get('entry_table_product');
				$data['entry_sorted'] = $this->language->get('entry_sorted');
      ...
      $url .= '&filter_category=' . urlencode(html_entity_decode($this->request->get['filter_category'], ENT_QUOTES, 'UTF-8'));
		...
		$data['footer'] = $this->load->controller('common/footer');
                                ...
                                */
                                $data['ocext_feed_generator_product_setting'] = $this->load->controller('feed/ocext_feed_generator_yamarket');
	...
	protected function getForm() {
                    ...
                    $data = $this->load->language('extension/module/youtube_product_video_edit');
    ...
    if ($this->config->get('config_editor_default')) {
		...
		$data['tab_attribute'] = $this->language->get('tab_attribute');
                                ...
                                */
                                if (version_compare(VERSION, '2.1') < 0) {
                                        ...
                                        $this->language->load('feed/ocext_feed_generator_yamarket');
                                }
				$data['tab_ocext_feed_generator'] = $this->language->get('tab_ocext_feed_generator');
                                if (version_compare(VERSION, '2.1') < 0) {
                                        $this->load->language('catalog/product');
                                } else {
		...
		$data['cpt_status'] = $this->config->get('cpt_status');
		if ($data['cpt_status']) {
			$this->load->model('extension/module/custom_product_tab');

			$data['tabs'] = $this->model_extension_module_custom_product_tab->getTabs(array('type' => 'regular'));
			$tabs = array();

			if (!isset($this->request->get['product_id'])) {
				if ($this->request->server['REQUEST_METHOD'] != 'POST') {
					foreach ($data['tabs'] as $idx => $tab) {
						if ($tab['type'] == 'regular' && $this->config->get('cpt_use_default_for_new')) {
							$tabs[$tab['tab_id']]['content'] = $this->model_extension_module_custom_product_tab->getTabDefaults($tab['tab_id']);
						} else {
							$tabs[$tab['tab_id']]['content'] = array();
						}
						$tabs[$tab['tab_id']]['status'] = 1;
					}
				} else {
					$tabs = isset($this->request->post['product_tab']) ? $this->request->post['product_tab'] : array();
				}
			} else if (isset($this->request->get['product_id']) && $this->request->server['REQUEST_METHOD'] != 'POST') {
				$statuses = $this->model_extension_module_custom_product_tab->getProductTabStatuses($this->request->get['product_id']);

				foreach ($data['tabs'] as $tab) {
					if ($tab['type'] == 'regular') {
						$tabs[$tab['tab_id']]['content'] = $this->model_extension_module_custom_product_tab->getProductTabContents($this->request->get['product_id'], $tab['tab_id']);
					} else {
						$tabs[$tab['tab_id']]['content'] = array();
					}
					$tabs[$tab['tab_id']]['status'] = isset($statuses[$tab['tab_id']]) ? $statuses[$tab['tab_id']] : 1;
				}
			} else {
				$tabs = isset($this->request->post['product_tab']) ? $this->request->post['product_tab'] : array();
			}

			$data['product_tab'] = $tabs;
		}
